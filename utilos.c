...
bool fslib_diropen(FSDirReader* rd, const char* path)
{
        ...
        */
        ...
        #if 0
        if(strchr(itempattern, '*') == NULL)
        #endif
        {
            strncat(itempattern, "/*", 2);
        }
        #if 0
            fprintf(stderr, "itempattern: <%s>\n", itempattern);
        #endif
        rd->findhnd = FindFirstFile(itempattern, &rd->finddata);
        mc_memory_free(itempattern);
        if(INVALID_HANDLE_VALUE == rd->findhnd)
        {
           return false;
        }
        return true;
...
bool fslib_dirread(FSDirReader* rd, FSDirItem* itm)
{
    ...
    memset(itm->name, 0, OSFN_PATHSIZE);
        ...
        return true;
    #else
        ok = FindNextFile(rd->findhnd, &rd->finddata);
        if(ok != 0)
        {
            if(INVALID_HANDLE_VALUE == rd->findhnd)
            {
            ...
            if(rd->finddata.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
            {
                itm->isfile = false;
                itm->isdir = true;
            }
            ...
            strcpy(itm->name, rd->finddata.cFileName);
            return true;
        }
    #endif
    return false;
