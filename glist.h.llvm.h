...
namespace llvm
{
    ...
    /// m_vecsize and m_veccapacity of the SmallVector, so it can be adjusted.
    ...
    class SmallVectorBase
    {
        protected:
            ...
            SmallVectorBase(void* FirstEl, size_t TotalCapacity) : m_beginx(FirstEl), m_veccapacity(static_cast<Size_T>(TotalCapacity))
            {
            }
        ...
        protected:
            ...
            void set_size(size_t N)
            {
                ...
                m_vecsize = static_cast<Size_T>(N);
            }
            ...
            void set_allocation_range(void* Begin, size_t N)
            {
                ...
                m_beginx = Begin;
                m_veccapacity = static_cast<Size_T>(N);
            }
    };
    ...
    protected:
        ...
        SmallVectorTemplateCommon(size_t sz) : Base(getFirstEl(), sz)
        {
        }
        ...
        bool isSmall() const
        {
            return this->m_beginx == getFirstEl();
        }

        /// Put this vector in a state of being small.
        void resetToSmall()
        {
            this->m_beginx = getFirstEl();
            this->m_vecsize = this->m_veccapacity = 0;// FIXME: Setting m_veccapacity to 0 is suspect.
        }
        ...
        {
            if constexpr(std::is_pointer_v<ItTy> && std::is_same_v<std::remove_const_t<std::remove_pointer_t<ItTy>>, std::remove_const_t<T>>)
            {
                if(From == To)
                    return;
    ...
    public:
        ...
        iterator begin()
        {
            return (iterator)this->m_beginx;
        }
        const_iterator begin() const
        {
            return (const_iterator)this->m_beginx;
        }
        iterator end()
        {
            return begin() + size();
    ...
    protected:
        ...
        SmallVectorTemplateBase(size_t sz) : SmallVectorTemplateCommon<T>(sz)
        {
        }
    ...
    template <typename T, bool TriviallyCopyable> T* SmallVectorTemplateBase<T, TriviallyCopyable>::mallocForGrow(size_t MinSize, size_t& NewCapacity)
    {
        return static_cast<T*>(SmallVectorBase<SmallVectorSizeType<T>>::mallocForGrow(this->getFirstEl(), MinSize, sizeof(T), NewCapacity));
    }

    // Define this out-of-line to dissuade the C++ compiler from inlining it.
    template <typename T, bool TriviallyCopyable> void SmallVectorTemplateBase<T, TriviallyCopyable>::moveElementsForGrow(T* NewElts)
    {
    ...
    // Define this out-of-line to dissuade the C++ compiler from inlining it.
    template <typename T, bool TriviallyCopyable> void SmallVectorTemplateBase<T, TriviallyCopyable>::takeAllocationForGrow(T* NewElts, size_t NewCapacity)
    {
    ...
    protected:
        ...
        template <typename It1, typename It2> static void uninitialized_copy(It1 I, It1 E, It2 Dest)
        {
            if constexpr(std::is_pointer_v<It1> && std::is_pointer_v<It2> && std::is_same_v<std::remove_const_t<std::remove_pointer_t<It1>>, std::remove_pointer_t<It2>>)
            {
    ...
    public:
        SmallVectorImpl() : SmallVectorImpl(8)
        {
        }
        ...
        void assignRemote(SmallVectorImpl&& RHS)
        {
            ...
            this->m_vecsize = RHS.m_vecsize;
            this->m_veccapacity = RHS.m_veccapacity;
            RHS.resetToSmall();
        }
    ...
    public:
        ...
        template <typename ItTy, typename = EnableIfConvertibleToInputIterator<ItTy>> void append(ItTy in_start, ItTy in_end)
        {
        ...
        template <typename ItTy, typename = EnableIfConvertibleToInputIterator<ItTy>> void assign(ItTy in_start, ItTy in_end)
        {
        ...
        template <typename U, typename = std::enable_if_t<std::is_convertible_v<U, T>>> void assign(ArrayRef<U> AR)
        {
    ...
    public:
        ...
        template <typename ItTy, typename = EnableIfConvertibleToInputIterator<ItTy>> iterator insert(iterator I, ItTy From, ItTy To)
        {
            // Convert iterator to elt# to avoid invalidating iterator when we reserve()
            size_t InsertElt = I - this->begin();
    ...
    template <typename T> void SmallVectorImpl<T>::swap(SmallVectorImpl<T>& RHS)
    {
        ...
        if(!this->isSmall() && !RHS.isSmall())
        {
            ...
            std::swap(this->m_vecsize, RHS.m_vecsize);
            std::swap(this->m_veccapacity, RHS.m_veccapacity);
            return;
        }
    ...
    class SmallVector : public SmallVectorImpl<T>, SmallVectorStorage<T, N>
    {
        public:
            ...
            explicit SmallVector(size_t sz) : SmallVectorImpl<T>(N)
            {
                this->resize(sz);
            }

            SmallVector(size_t sz, const T& Value) : SmallVectorImpl<T>(N)
            {
                this->assign(sz, Value);
            }

            template <typename ItTy, typename = EnableIfConvertibleToInputIterator<ItTy>> SmallVector(ItTy S, ItTy E) : SmallVectorImpl<T>(N)
            {
                this->append(S, E);
            }

            template <typename RangeTy> explicit SmallVector(const iterator_range<RangeTy>& R) : SmallVectorImpl<T>(N)
            {
            ...
            template <typename U, typename = std::enable_if_t<std::is_convertible_v<U, T>>> explicit SmallVector(ArrayRef<U> A) : SmallVectorImpl<T>(N)
            {
            ...
            void dangerOrphanData()
            {
                //this->m_beginx = nullptr;
                this->m_vecsize = this->m_veccapacity = 0;
            }
    };
...
// warning since we are only using these types for the static assertions below.
    ...
    static_assert(sizeof(SmallVector<char, 0>) == sizeof(void*) * 2 + sizeof(void*), "1 byte elements have word-sized type for size and capacity");
    ...
    {
        std::string Reason = "SmallVector unable to grow. Requested capacity (" + std::to_string(MinSize) + ") is larger than maximum value for size type (" + std::to_string(MaxSize) + ")";
    ...
    /// space, and happens to allocate precisely at m_beginx.
    ...
    void SmallVectorBase<Size_T>::grow_pod(void* FirstEl, size_t MinSize, size_t TSize)
    {
        ...
        if(m_beginx == FirstEl)
        {
            ...
            memcpy(NewElts, this->m_beginx, size() * TSize);
        }
        else
        {
            // If this wasn't grown from the inline copy, grow the allocated space.
            NewElts = realloc(this->m_beginx, NewCapacity * TSize);
            if(NewElts == FirstEl)
...
#define CflatInvokeDtor(pClassName, pPtr) (pPtr)->~pClassName()
        ...
        public:
            StackVector() : mFirst(&mData[0]), mEnd(mFirst), mSize(0u)
            {
            }
            ...
            {
                assert(pIterator >= mFirst && pIterator <= mEnd);
...
class GenericList
{
    ...
    private:
        template <typename OtherT> void moveFrom(OtherT* other)
        {
            m_listvector = new VecType(*other->m_listvector);
        }

    public:
        GenericList() : GenericList(4)
        {
        }

        GenericList(GenericList&& other) noexcept
        {
            moveFrom(&other);
        }
        ...
        {
            m_listvector = new VecType(initialsize);
        }
